...
def vac_processing():
     ...
     df = pd.read_csv('vac-keywords-TFIDF.csv')
     ...
     for index in range(0,(len(KEYWORD1)-1)):
          ...
          for cvcategory in Categories:
               ...
               if len(temp)>1:
                    ...
                    group.append({'Folder-Label':cvcategory,'Job-Vacancy':temp,'Keywords':KEYWORD1[index]})
     
     # with open('vac-clusters.csv', 'w', newline='') as csv_file:
     #      writer = csv.writer(csv_file) 
     #      for item in group:
     #           for key in item:
     #                if key=='label':
     #                     print(item[key])
     file = open('vac-clusters.csv', 'w', newline ='')
 
     with file:
          # identifying header 
          header = ['Folder-Label','Job-Vacancy', 'Keywords']
